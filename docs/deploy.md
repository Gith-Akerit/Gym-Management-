...
| `PILOT_MODE` | | `1` เพื่อเปิดโหมดทดลอง (ดูหัวข้อข้างล่าง) ลบบรรทัดทิ้งเมื่อจะเปิดใช้จริง |
| `PROMPTPAY_ID` | ✔ ยกเว้นโหมดทดลอง | เบอร์มือถือ / เลขบัตรประชาชน 13 หลัก / e-Wallet 15 หลัก ของบัญชีที่รับเงิน **เป็นข้อมูลส่วนตัวของเจ้าของยิม ห้ามคอมมิตลง repo** |
| `SMTP_HOST` `MAIL_FROM` | ✔ ยกเว้นโหมดทดลอง | `smtp-relay.brevo.com` และที่อยู่ผู้ส่งที่ยืนยันกับ Brevo แล้ว ดูหัวข้อ "อีเมล OTP" ข้างล่าง |
...
ตั้ง `PILOT_MODE=1` แล้วระบบขึ้นได้ทันทีโดย**ไม่ต้องมี PromptPay ID และไม่ต้องมีบริการส่งอีเมล** เหมาะกับการให้ผู้ใช้จริงกลุ่มเล็กลองใช้ก่อนตัดสินใจเปิดเต็มรูปแบบ

| | โหมดทดลอง | เปิดใช้จริง |
|---|---|---|
| เข้าสู่ระบบ | อีเมล + OTP เหมือนเดิม แต่**รหัสแสดงในหน้าแอดมิน** เจ้าหน้าที่บอกรหัสให้สมาชิก | ส่งรหัสทางอีเมล |
| ซื้อแพ็กเกจ | ไม่มีหน้าซื้อ **แอดมินมอบแพ็กเกจให้จากหน้าสมาชิกรายคน** | PromptPay QR + ส่งสลิป + แอดมินอนุมัติ |
| คิวตรวจสลิป | ซ่อน แทนด้วยแท็บ "รหัส OTP" | แสดงตามปกติ |
| สมัคร/จัดการสมาชิก, ตั้งค่าแพ็กเกจ, QR เช็คอิน, นับครั้ง, ใครใกล้หมดอายุ | ใช้ได้ครบ | ใช้ได้ครบ |
| แถบเตือน "โหมดทดลอง" | แสดงบนทุกหน้าของแอดมินและพนักงาน | ไม่มี |

**รหัส OTP ไม่ได้ถูกบันทึกลงฐานข้อมูล** เก็บไว้ในหน่วยความจำของเซิร์ฟเวอร์ อายุ 5 นาทีเท่าเดิม และหายไปเมื่อรีสตาร์ต ตารางในฐานข้อมูลยังเก็บเฉพาะ HMAC เหมือนโหมดปกติ

### ล็อกอินครั้งแรกของแอดมิน

โหมดทดลองอ่านรหัสจากหน้าแอดมิน แต่แอดมินคนแรกยังเข้าหน้านั้นไม่ได้ จึงมีคำสั่งบนเซิร์ฟเวอร์สำหรับกรณีนี้โดยเฉพาะ:

```bash
cd /srv/gym && docker compose exec app npm --silent run pilot:code -- owner@example.com
```

`--silent` ตัด banner ของ npm ออก บรรทัดแรกที่ขึ้นจึงเป็นรหัสเลย สคริปต์พิมพ์รหัส 6 หลักกับลิงก์ใช้ครั้งเดียวออกมา เปิดลิงก์บนมือถือแล้วกรอกรหัสภายใน 5 นาที **ต้องใช้ลิงก์ที่พิมพ์ออกมา** ไม่ใช่หน้าที่เปิดค้างไว้ เพราะการกด "ขอรหัสเข้าใช้งาน" ทุกครั้งจะยกเลิกรหัสก่อนหน้าเสมอ

ข้อจำกัดของคำสั่งนี้: ทำงานเฉพาะเมื่อ `PILOT_MODE=1` และเฉพาะบัญชีที่เป็น `admin` อยู่แล้ว รหัสของสมาชิกและพนักงานดูจากแท็บ "รหัส OTP" เท่านั้น ทุกครั้งที่ออกรหัสจะถูกบันทึกใน audit log (`auth.pilot_code_cli`) ส่วนตัวรหัสไม่ถูกบันทึกที่ไหนเลย
ตัวติดตั้ง `--pilot` เรียกคำสั่งนี้ให้อัตโนมัติตอนจบการติดตั้ง จึงได้รหัสแรกมาพร้อมกับ URL

**เห็นรหัสได้เฉพาะ `admin`** ไม่ใช่ `staff` เพราะการอ่านรหัสของใครคนหนึ่งมีค่าเท่ากับการเข้าสู่ระบบเป็นคนนั้นได้ ถ้ายิมต้องการให้พนักงานหน้าเคาน์เตอร์อ่านรหัสได้ด้วย แจ้งได้ เป็นการแก้บรรทัดเดียว

**แพ็กเกจที่แอดมินมอบให้จะไม่ถูกนับเป็นรายได้** คำสั่งซื้อถูกทำเครื่องหมาย `manual_grant` และหน้าสรุปยอดขายแยกไว้ในคอลัมน์ `manual_grants` เพื่อให้ยอดที่เอาไปกระทบกับ statement ธนาคารตรงกับเงินที่เข้าจริง
...
**ข้อมูลที่เกิดในโหมดทดลองใช้ต่อได้ทั้งหมด** สมาชิก แพ็กเกจ สิทธิ์ที่แอดมินมอบให้ และประวัติเช็คอิน ไม่ต้องติดตั้งใหม่และไม่ต้องล้างฐานข้อมูล

ถ้าเปิด `PILOT_MODE=1` ทิ้งไว้ทั้งที่กรอก PromptPay หรือ SMTP แล้ว ตอนบูตจะมี log เตือนบอกว่าค่าไหนถูกข้าม:

```json
{"event":"pilot_mode","ignored":["PROMPTPAY_ID"],"warning":"PROMPTPAY_ID is configured but will NOT be used while PILOT_MODE=1. Remove PILOT_MODE to go live."}
```

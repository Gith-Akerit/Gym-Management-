...
| `OTP_SECRET` | ✔ | สุ่ม 32 ไบต์: `node -e "console.log(require('crypto').randomBytes(32).toString('hex'))"` เซ็น session, โทเคนเช็คอิน และ **QR บนบัตรสมาชิกทุกใบ** — **เปลี่ยนเมื่อไหร่ ทุกคนหลุดออกจากระบบ และบัตรที่ส่งให้สมาชิกไปแล้วทุกใบใช้ไม่ได้** ตั้งครั้งเดียวแล้วอย่าแตะ |
...
`OTP_SECRET` Render สุ่มให้เองครั้งเดียวแล้วเก็บไว้ **อย่ากด regenerate** เพราะทุกคนจะหลุดออกจากระบบและบัตรสมาชิกทุกใบที่ส่งไปแล้วจะใช้ไม่ได้
...
บัตรสมาชิกเป็นไฟล์ PNG ขนาด 1080 × 1350 ที่เซิร์ฟเวอร์วาดสด ๆ ทุกครั้งที่ขอ ไม่ได้เก็บเป็นไฟล์ จึงไม่ต้องสำรอง QR บนบัตรเป็นโทเคนถาวรที่เซ็นด้วย `OTP_SECRET` และมีเลขที่บัตร (`card_version`) อยู่ข้างใน กด "ออกบัตรใหม่" แล้วเลขนั้นเพิ่มขึ้น บัตรใบเก่าจึงใช้ไม่ได้ทันทีโดยไม่ต้องไปตามลบรูปที่ไหน

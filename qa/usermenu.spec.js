...
test('MENU-14 a screen reader hears the name and the note as two things', async ({ page }) => {
  await signIn(page, ADMIN);
  await openUserMenu(page);

  // The computed accessible name and description, not the attributes: what
  // matters is the string a screen reader ends up announcing, which is worked
  // out from several of them together.
  const rows = page.locator('#usermenu-panel [role="menuitem"]');
  const seen = [];
  for (let i = 0; i < await rows.count(); i++) {
    const row = rows.nth(i);
    seen.push({
      name: await row.evaluate(el => el.getAttribute('aria-label')
        ?? el.textContent.replace(/s+/g, ' ').trim()),
      describedby: await row.getAttribute('aria-describedby'),
      note: await row.evaluate(el => {
        const id = el.getAttribute('aria-describedby');
        return id ? document.getElementById(id)?.textContent.trim() ?? null : null;
      }),
    });
  }
  console.log('MENU-14 what a screen reader announces:\n' + JSON.stringify(seen, null, 1));

  for (const row of seen) {
    // The note must not be glued onto the end of the name. Before this was
    // split, the name came out as "แจ้งปัญหาจับภาพหน้าจอนี้ให้อัตโนมัติ" —
    // one run-on string that is not a sentence anybody wrote.
    expect(row.name, `"${row.name}" has the note glued onto the label`)
      .not.toMatch(/จับภาพหน้าจอนี้ให้อัตโนมัติ|เปิดไฟล์ PDF/);
  }
  // Playwright's own accessible-name computation agrees, for the two rows that
  // carry a note at all.
  await expect(page.getByRole('menuitem', { name: 'แจ้งปัญหา', exact: true }))
    .toHaveAccessibleName('แจ้งปัญหา');
  await expect(page.getByRole('menuitem', { name: 'คู่มือการใช้งาน', exact: true }))
    .toHaveAccessibleName('คู่มือการใช้งาน');

  // And the note is still announced rather than thrown away.
  const described = seen.filter(r => r.describedby && r.note);
  console.log('MENU-14 rows whose note is pointed at separately:',
    JSON.stringify(described.map(r => `${r.name} -> ${r.note}`)));
  expect(described.length, 'no row points at its note').toBeGreaterThanOrEqual(2);
});

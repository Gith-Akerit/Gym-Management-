...
/** Approved, not built. Shown on purpose so "where would I report this?" has an answer. */
...
test('MENU-01 the menu offers exactly what the person is allowed to open', async ({ page }) => {
  ...
  expect(ownerOnly.sort()).toEqual(['ข้อมูลยิม', 'ผู้ใช้และสิทธิ์', 'แพ็กเกจ'].sort());
  expect(staffOnly, 'a member of staff is offered something the owner is not').toEqual([]);

  // A greyed-out row that never works teaches staff to keep trying it. The two
  // that are disabled must be the two the team said, and nothing else.
  for (const [role, row] of Object.entries(seen)) {
    ...
    expect(off.sort(), `${role}: something unexpected is greyed out`).toEqual([...NOT_YET].sort());
  }
});

test('MENU-02 the two unbuilt rows say so and truly do nothing', async ({ page }) => {
  await signIn(page, ADMIN);
  await openUserMenu(page);
  const rows = (await menuRows(page)).filter(r => NOT_YET.includes(r.label));
  console.log('MENU-02 the rows that are not built yet:\n' + JSON.stringify(rows, null, 1));
  for (const row of rows) {
    expect(row.disabled, `"${row.label}" can be pressed`).toBe(true);
    expect(row.sub, `"${row.label}" does not say why it cannot be pressed`).toBeTruthy();
    expect(row.focusable, `"${row.label}" is still in the keyboard order`).toBe(false);
  }

  // Pressing one must not navigate, must not close the menu behind the user's
  // back, and must not throw.
  const errors = [];
  page.on('pageerror', e => errors.push(String(e)));
  const before = page.url();
  // A `disabled` button takes no pointer events at all, so Playwright cannot
  // click it — which is itself the answer. The event is dispatched directly
  // instead, to prove nothing is listening behind the attribute.
  const dispatched = await page.evaluate(labels => labels.map(label => {
    const row = [...document.querySelectorAll('#usermenu-panel [role="menuitem"]')]
      .find(el => el.textContent.includes(label));
    row.dispatchEvent(new MouseEvent('click', { bubbles: true }));
    return { label, disabled: row.disabled };
  }), NOT_YET);
  console.log('MENU-02 dispatching a click straight at them:', JSON.stringify(dispatched));
  console.log('MENU-02 after pressing both: menu still open ->',
    await page.locator('#usermenu-panel').isVisible(), '· url moved ->', page.url() !== before,
    '· javascript errors ->', JSON.stringify(errors));
  expect(page.url(), 'a row that is not built navigated somewhere').toBe(before);
  expect(errors, 'pressing a disabled row threw').toEqual([]);

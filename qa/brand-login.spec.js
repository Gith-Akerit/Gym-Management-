...
test('BRAND-UI-05 a button still looks like a button whatever colour the gym picked', async ({ page }) => {
  ...
  for (const colour of ['#05603A', '#C81E1E', '#F2C200', '#FFFFFF', '#808080']) {
    ...
    // `--brand-surface` landing is not the same as the button having finished
    // moving: `.btn` carries `transition:background`, so for a frame or two
    // `getComputedStyle` hands back a colour part-way between the old fill and
    // the new one -- a colour no token ever named -- measured against an ink
    // that already changed. Two frames that agree mean the transition is done.
    // (Helper taken from the team's `tests/ui/09-tokens.spec.js`.)
    ...
    // The hover state paints the button a second colour, and `85bfa2d` changed
    // its border too. A button that is bounded at rest and loses its edge the
    // moment somebody points at it is worse than one that never had an edge,
    // so the pointed-at state is measured as well as the resting one.
    await page.locator('.btn.primary').hover();
    await page.waitForFunction(() => {
      const el = document.querySelector('.btn.primary');
      const now = getComputedStyle(el).backgroundColor;
      const same = window.__hoverFill === now;
      window.__hoverFill = now;
      return same;
    });
    ...
    report[colour] = {
      ...
      hover_outline: Math.max(ratio(hovered.fill, hovered.card),
        hovered.width === '0px' ? 0 : ratio(hovered.border, hovered.card)),
      hover_label: ratio(hovered.label, hovered.fill),
  ...
  for (const [colour, row] of Object.entries(report)) {
    ...
    expect(row.hover_label, `${colour}: pointed at, the label is ${row.hover_label}:1`)
      .toBeGreaterThanOrEqual(4.5);
    expect(row.hover_outline, `${colour}: pointed at, the button's outline is ${row.hover_outline}:1`)
      .toBeGreaterThanOrEqual(3);
  }
});

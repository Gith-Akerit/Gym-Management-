...
export const AUDIT = `(() => {
  ...
  for (const el of all) {
    ...
    if (!own) continue;
    // A swatch that demonstrates a colour is not text anybody reads, and it is
    // marked as such. The contrast table on "ตั้งค่ายิม" shows the border
    // colour in an aria-hidden square, which this otherwise reported as
    // 14px text failing 4.5:1 when its real bar is the 3:1 of a border.

...
/**
 * A save the way the screen makes one.
 *
 * `version` is a required field: the server refuses a save it cannot check
 * against anybody else's (BRAND-07), so a probe that means to change a setting
 * reads the row first, exactly like the browser does.
 */
async function save(call, token, fields, status) {
  const version = (await call('get', '/gym/settings', token)).body.version;
  const res = call('put', '/gym/settings', token, { ...fields, version });
  return status === undefined ? res : res.expect(status);
}
...
test('BRAND-02 what the two public endpoints hand to a passer-by', async t => {
  ...
  await save(call, owner, { line_id: '@qagym', brand_short: 'คิว' }, 200);
...
test('BRAND-04 the colour field refuses what is not a colour, and keeps what was there', async t => {
  ...
  const owner = await signIn('owner@example.test', 'admin');
  await save(call, owner, { color_primary: '#C81E1E' }, 200);
  ...
  for (const [name, value] of Object.entries(attempts)) {
    const res = await save(call, owner, { color_primary: value });
...
test('BRAND-05 changing the colour never changes the card a member is carrying', async t => {
  ...
  for (const colour of ['#C81E1E', '#F2C200', '#FFFFFF', '#000000', '#808080']) {
    await save(call, owner, { color_primary: colour }, 200);
...
test('BRAND-06 what the settings screen previews is what the card is painted with', async t => {
  ...
  for (const colour of ['#C81E1E', '#F2C200', '#1B4FD8']) {
    const saved = await save(call, owner, { color_primary: colour }, 200);

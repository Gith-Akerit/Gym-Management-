...
| `qa-counter-migration.test.js` | migration 012 บนฐานที่มีเรื่องอยู่แล้ว: seed จากเลขที่เคยออก · ลบทั้งตารางแล้วไม่เริ่มใหม่ · ถอยแล้วขึ้นใหม่ก็ยังเดินหน้า |
...
## สถานะบน `release/pilot` (`596e141`)

| ชุด | ผล |
|---|---|
| `node --test qa/*.test.js` | **65/65** |
| `npm test` (ชุดของทีมพัฒนา) | **186/186** |
| `PLAYWRIGHT_CHANNEL=msedge npm run test:ui` | **59/59** สองรอบติด — `scanReady()` แก้อาการแกว่งที่รากแล้ว |
...
| `npx playwright test --config qa/playwright.qa.config.js` | **20/20** |
...
- **แก้ `.jsx` แล้วต้อง `npm run build` ก่อนรัน playwright** เซิร์ฟเวอร์ทดสอบเสิร์ฟ `dist/` ไม่ได้ build ให้เอง
  รอบนี้ผมเกือบรายงานว่า `aria-label` ของแถวเมนูไม่ทำงาน ทั้งที่อ่าน bundle เก่าจากคอมมิตก่อนอยู่

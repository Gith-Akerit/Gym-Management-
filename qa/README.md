...
## สถานะบน `release/pilot` (`ba5fdf6`)

| ชุด | ผล |
|---|---|
| `node --test qa/*.test.js` | **42/44** — ที่แดงคือ `BRAND-03` และ `BRAND-07` ซึ่งเป็น probe ที่เฝ้าบั๊กที่ยังไม่ได้แก้ จะเขียวเมื่อทีมพัฒนาแก้ |
| `npm test` (ชุดของทีมพัฒนา) | **164/164** ยืนยันเองแล้วที่ `ba5fdf6` |
| `PLAYWRIGHT_CHANNEL=msedge npm run test:ui` | **40/40** บน Edge ยืนยันเองแล้วที่ `ba5fdf6` |
...
| `npx playwright test --config qa/playwright.qa.config.js` | **0/4** — ทั้งสี่คือ probe ที่เฝ้า BUG-BRAND-01/02/03 บนหน้าล็อกอินและจอสแกน |

...
function gym(t) {
  ...
  // The CLI that issues a set-password link runs as a separate process and
  // stamps its rows with the real clock, so the fixture starts from the real
  // clock too. Frozen at a fixed date, the two disagreed by however far the
  // wall clock had moved past 09:00 Bangkok, and `tick(DAY)` then landed
  // *before* an expiry the CLI had written -- a probe that passed all morning
  // and failed all afternoon, for no reason in the product.
  let time = Date.now();
...
test('SETPW-01 the link the owner is sent works once, and then never again', async t => {
  ...
  // Both rows must be there. Their order is not asserted: the two are written
  // milliseconds apart by two different processes, so ordering by `created_at`
  // is a coin toss rather than a fact about the product.
  assert.deepEqual(trail.map(r => r.action).sort(),
    ['user.password_link_cli', 'user.password_set_by_link'].sort());

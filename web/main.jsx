...
function Console({ user, gym, brand, branding, onBrandingChange, onLogout, onAuthError, onSignedOut }) {
  ...
  return <Shell brand={brand} branding={branding} user={user} tabs={tabs} tab={tab}
    ...
    onLogout={onLogout} onPick={pick} pilot={pilot} wide={tab === 'branding' || tab === 'users'} pending={pending}>
    {reportUi}{passwordUi}
    {banner}
    ...
    {/* "ผู้ใช้และสิทธิ์" in the corner menu is this screen with its accounts
        tab already open, not a second copy of it: the reports and the accounts
        are settings of this gym the way its colours are, and two doors onto
        two versions of one screen is how they drift apart. */}
    {(tab === 'branding' || tab === 'users') && <GymBranding role={user.role} onAuthError={onAuthError}
      initialTab={tab === 'users' ? 'users' : 'brand'}
      signedInAs={user.email} onSignedOut={onSignedOut}
